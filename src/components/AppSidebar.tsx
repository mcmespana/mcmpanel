...
export function AppSidebar({
  ...
  const handleToggleReview = () => {
    if (!appReviewMode) {
      if (!confirm(
        '¿Activar Modo revisión de app?\n\nSe ocultarán Cantoral y todas las secciones de Comunica en TODOS los perfiles. Los valores actuales se guardarán como backup para restaurarlos al desactivarlo.',
      )) return;
      onToggleAppReviewMode(true);
    } else {
      onToggleAppReviewMode(false);
    }
  };
  ...
  return (
    <Sidebar className="border-r border-border/40 bg-sidebar">
      ...
      <button
        ...
        title={appReviewMode
          ? 'Modo revisión ACTIVO — toca para desactivar y restaurar tabs ocultas'
          : 'Activar Modo revisión de app (oculta Cantoral y Comunica)'}
        className={cn(
          "group flex items-center gap-2 w-full border-b border-border/40 transition-colors",
      ...
      >
        {appReviewMode ? (
          <ShieldAlert className="w-4 h-4 text-warning flex-shrink-0 animate-pulse-subtle" />
        ) : (
          <ShieldCheck className="w-4 h-4 text-muted-foreground/60 group-hover:text-foreground flex-shrink-0" />
        )}
        {!collapsed && (
          <>
            <div className="flex-1 min-w-0 text-left">
              <p className={cn(
                ...
                appReviewMode ? "text-warning" : "text-muted-foreground group-hover:text-foreground",
              )}>
                {appReviewMode ? 'Modo revisión ACTIVO' : 'Modo revisión de app'}
              </p>
              <p className="text-[10px] text-muted-foreground/70 mt-0.5 leading-none truncate">
                {appReviewMode
                  ? 'Cantoral y Comunica ocultos · toca para apagar'
                  : 'Oculta Cantoral y Comunica al instante'}
              </p>
            </div>
            <span
              className={cn(
                "flex-shrink-0 inline-flex h-5 w-9 items-center rounded-full transition-colors border",
                appReviewMode
            ...
            >
              <span
                className={cn(
                  "h-3.5 w-3.5 rounded-full bg-background shadow transition-transform",
                  appReviewMode ? "translate-x-[18px]" : "translate-x-[2px]",
                )}
              />
            </span>
          </>
